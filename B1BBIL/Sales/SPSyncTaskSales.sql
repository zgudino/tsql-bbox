IF EXISTS
    (SELECT *
     FROM sys.objects
     WHERE object_id = OBJECT_ID(N'SPSyncTaskSales')
         AND OBJECTPROPERTY(OBJECT_ID(N'SPSyncTaskSales'), N'IsProcedure') = 1)

    DROP PROCEDURE SPSyncTaskSales

GO

CREATE PROCEDURE SPSyncTaskSales
AS
    DECLARE @Retries int
    SET @Retries = 1

IF EXISTS
    (SELECT *
     FROM sys.objects
     WHERE object_id = OBJECT_ID(N'SPSyncTaskDeliveries')
         AND OBJECTPROPERTY(OBJECT_ID(N'SPSyncTaskDeliveries'), N'IsProcedure') = 1)

    DROP PROCEDURE SPSyncTaskDeliveries

GO

CREATE PROCEDURE SPSyncTaskDeliveries
AS
    DECLARE @Retries int
    SET @Retries = 1
